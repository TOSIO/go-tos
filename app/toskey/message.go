...
`,
	...
	Action: func(ctx *cli.Context) error {
		...
		if err != nil {
			Fatalf("Failed to read the keyfile at '%s': %v", keyfilepath, err)
		}
		...
		if err != nil {
			Fatalf("Error decrypting key: %v", err)
		}
...
It is possible to refer to a file containing the message.`,
	...
	Action: func(ctx *cli.Context) error {
		...
		if !common.IsHexAddress(addressStr) {
			Fatalf("Invalid address: %s", addressStr)
		}
		address := common.HexToAddress(addressStr)
		signature, err := hex.DecodeString(signatureHex)
		if err != nil {
			Fatalf("Signature encoding is not hexadecimal: %v", err)
		}
...
func getMessage(ctx *cli.Context, msgarg int) []byte {
	if file := ctx.String("msgfile"); file != "" {
		if len(ctx.Args()) > msgarg {
			Fatalf("Can't use --msgfile and message argument at the same time.")
		}
		msg, err := ioutil.ReadFile(file)
		if err != nil {
			Fatalf("Can't read message file: %v", err)
		}
		return msg
	} else if len(ctx.Args()) == msgarg+1 {
		return []byte(ctx.Args().Get(msgarg))
	}
	Fatalf("Invalid number of arguments: want %d, got %d", msgarg+1, len(ctx.Args()))
	return nil
}
